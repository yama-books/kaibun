...
const wrapperIds=["tell-topic","read-muyo-topic","state-rubeno-topic","say-topic","hear-topic","state-topic","narrate-topic","judge-topic","interpret-topic"];
const quoteMarks=[["「","」"],["『","』"],["〈","〉"],["《","》"],["【","】"],["〔","〕"],["［","］"],["（","）"],["〝","〟"],["“","”"],["‘","’"]];
...
for(const sid of sourceIds){
  ...
  for(let i=0;i<wrapperIds.length;i++){
    ...
    const depth=i+1,q=quoteMarks[i%quoteMarks.length];
  ...
  generated.push({
    id:`quote-simple-${sid.replace(/^event-/,"")}`,
    ...
    wrapper_order:"v0.72-compatible fixed sequence",stages
  });
}

if(fixture.version!=="0.87")errors.push(`fixture version ${fixture.version}, expected 0.87`);
...
if(JSON.stringify(fixture.wrapper_sequence)!==JSON.stringify(wrapperIds))errors.push("wrapper sequence drift");
...
if(projected>=0.60)errors.push(`projected kinship share ${projected} must be <0.60`);

if(errors.length){
  console.error("Non-kinship quote expansion validation failed:\n"+errors.map(x=>"- "+x).join("\n"));
  process.exit(1);
}
console.log("OK: non-kinship quote expansion v87");
console.log(`Families=4, stages=40, projected kinship share=${(projected*100).toFixed(1)}%`);
